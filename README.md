...
- **Строгая опора на контекст**: ассистент использует только ваши документы, не придумывая фактов.  
- **Поддержка форматов**: `.txt`, `.pdf`, `.docx`.  
- **Современный веб-интерфейс** на Streamlit:
  - История диалога с красивым оформлением в тёмных тонах.
  - Встроенная форма обратной связи для студентов.
  - Защищённый вход по логину и паролю.
- **Лёгкое развёртывание** через Docker.
...
1. **Индексация** (`ingest.py`)  
   - Документы из папки `data/` загружаются и разбиваются на смысловые фрагменты.  
...
2. **Ответ на вопрос** (`rag_engine.py` + `app_streamlit.py`)  
   ...
   - ChromaDB находит до 4 наиболее похожих фрагментов из базы.  
   - Контекст передаётся в GigaChat с инструкцией отвечать только на его основе.  
   - Ответ возвращается в чат, при необходимости с указанием источников.
...
Откройте браузер по адресу [http://localhost:8501](http://localhost:8501), войдите под учётными данными и начинайте задавать вопросы.
...
### 1. Подготовьте `.env` и поместите документы

Создайте файл `.env` с ключами (как в шаге 3) и скопируйте ваши учебные материалы в папку `data/`.
...
### 3. Выполните индексацию внутри контейнера

```bash
docker-compose exec assistant python ingest.py
```

Веб-интерфейс будет доступен на порту `8501`.
...
### Изменение количества найденных фрагментов
...
|----------|-------------------|---------|
| `402 Payment Required` при индексации | Используются платные эмбеддинги GigaChat | Убедитесь, что в коде применяются локальные эмбеддинги HuggingFace |
| `AuthenticationError` | Неверный или просроченный ключ авторизации GigaChat | Сгенерируйте новый Authorization Key и обновите `.env` |
| `ModuleNotFoundError` | Не установлены все зависимости | Выполните `pip install -r requirements.txt` |
| Предупреждения при загрузке модели | Отсутствует токен HuggingFace Hub | Можно игнорировать или добавить `HF_TOKEN` в `.env` |
